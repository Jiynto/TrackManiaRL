...
class Agent:
    def __init__(self, alpha=0.0003, beta=0.0003, input_dims=[78],
                 max_action=1, gamma=.99, n_actions=2, max_size=1000000, tau=0.005,
    ...
    def learn(self):
        ...
        # Updates for the Value Network
        with tf.GradientTape() as tape:

            # current values from the two value networks when given our memory states.
            value = tf.squeeze(self.value(states), 1)
            value_ = tf.squeeze(self.target_value(states_), 1)

            # current action log probability pairs.
            current_poicy_actions, log_probs = self.actor.sample_normal(states, reparameterize=False)
            log_probs = tf.squeeze(log_probs, 1)

            # current critic values for state action pairs.
            q1_new_policy = self.critic_1(states, current_poicy_actions)
            q2_new_policy = self.critic_2(states, current_poicy_actions)
            critic_values = tf.squeeze(tf.math.minimum(q1_new_policy, q2_new_policy), 1)

            # Qmin(s,a) - logpi(a|s)
            value_target = critic_values - log_probs

            # mean squared loss of the network.
            # 0.5 * (V(s) - Qmin(s,a) - logpi(a|s))^2
        ...
        # Updates for the Actor network
        with tf.GradientTape() as tape:
            # reparameterization is applied here in source paper.
            # might want to apply it later, need to look into it.

            # new action log probability pairs from the memory states.
            new_policy_actions, log_probs = self.actor.sample_normal(states, reparameterize=True)
            log_probs = tf.squeeze(log_probs, 1)

            # current critic values for memory states and new action pairs.
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            # q_hat = r_scaled + gamma*target_value_network(states as a result of actions, i.e St+1)
            q_hat = self.scale * rewards + self.gamma * value_ * (1 - done)

            # currently returned critic values for past states and actions.
            q1_old_policy = tf.squeeze(self.critic_1(states, actions), 1)
            q2_old_policy = tf.squeeze(self.critic_2(states, actions), 1)

            # mean squared loss of with old policy.
            # 0.5 * (Q(s,a) - Q^(s|a))^2
        ...
        # Update Target Value Network to match Value Network
